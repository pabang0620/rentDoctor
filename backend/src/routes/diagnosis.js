    ...
    if (contractInfo.status === 'expired' || contractInfo.status === 'today') {
      actions.push({
        ...
        steps: [
          ...
          '법원 도착 후 "임차권등기명령 신청하러 왔습니다"라고 말하면 직원이 안내합니다 (인지대 약 15,000원 수준 — 변경될 수 있으니 방문 전 법원에 확인하세요)',
    ...
    if (!contractInfo || ['safe', 'soon'].includes(contractInfo.status)) {
      ...
      actions.push({
        ...
        steps: [
          ...
          '"임차권등기명령 신청하러 왔습니다"라고 말하면 됩니다 (인지대 약 15,000원 수준 — 변경될 수 있으니 방문 전 법원에 확인하세요)'
        ]
      })
    }
  }
