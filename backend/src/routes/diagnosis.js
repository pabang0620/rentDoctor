    ...
    if (contractInfo.status === 'expired' || contractInfo.status === 'today') {
      actions.push({
        ...
        steps: [
          ...
          '챙겨갈 서류: 임대차계약서 사본, 주민등록등본, 건물 등기부등본 (각 1통 — 주민센터·등기소 발급, 비용은 발급 시점에 확인하세요)',
          '법원 도착 후 "임차권등기명령 신청하러 왔습니다"라고 말하면 직원이 안내합니다 (인지대 등 비용은 법원에서 확인하세요)',
          '접수 후 등기부등본에 임차권이 등록됩니다 (처리 기간은 법원 상황에 따라 다를 수 있음)'
        ],
      })
      ...
      actions.push({
        ...
        steps: [
          ...
          '작성 내용: ①임대차 계약 기간과 보증금액 ②보증금 반환 요청 ③반환 기한 명시 (통보 후 7~14일 이내 권고)',
          'A4용지에 작성 후 동일 내용 3부 출력 → 가까운 우체국 방문 → "내용증명 보내러 왔습니다"라고 말하면 됩니다',
          '수령증은 반드시 보관하세요 — 집주인이 수령 거부해도 법적 효력은 유지됩니다 (우편 요금은 발송 시점에 확인)'
        ]
      })
  ...
  if (!checks.registrationAndDate) {
    actions.push({
      ...
      steps: [
        ...
        '비용 무료 — 전입신고 다음 날 0시부터 대항력이 발생합니다 (처리 시간은 주민센터 상황에 따라 다를 수 있음)'
      ]
    })
  }
    ...
    if (!contractInfo || ['safe', 'soon'].includes(contractInfo.status)) {
      actions.push({
        ...
        steps: [
          ...
          '작성: ①임대차 계약 기간과 보증금액 ②반환 요청 및 기한 명시 (통보일로부터 7~14일 이내 권고) ③내 연락처',
      ...
      actions.push({
        ...
        steps: [
          ...
          '"임차권등기명령 신청하러 왔습니다"라고 말하면 됩니다 (인지대 등 비용은 법원에서 확인하세요)'
        ]
      })
    }
  }
  ...
  if (!checks.noHugInsurance) {
    ...
    actions.push({
      ...
      steps: [
        ...
        '가입 조건(계약 기간, 주택가격 기준 등)은 변경될 수 있습니다 — 콜센터에서 본인 상황에 맞게 안내받으세요',
        '가입 가능하다면 즉시 신청하세요 — 연간 보험료는 보증금의 약 0.1~0.4% 수준이나, 정확한 요율은 HUG 가입 시 안내됩니다'
      ],
      contact: { name: '주택도시보증공사(HUG)', phone: '1566-9009' }
    })
  }
